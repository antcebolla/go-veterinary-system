...
type VeterinaryCenter struct {
	...
	Adress        string         `json:"address" gorm:"not null"`
...
func (vc *VeterinaryCenter) ValidateAndFormat() error {
	...
	vc.Adress = strings.TrimSpace(vc.Adress)
	vc.Phone = strings.TrimSpace(vc.Phone)
	if vc.Name == "" || vc.Adress == "" {
		return gorm.ErrInvalidData
	}
	if len(vc.Name) < minimal_name_length || len(vc.Adress) < minimal_address_length {
		return gorm.ErrInvalidData
	}
